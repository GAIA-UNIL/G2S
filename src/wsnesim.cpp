...
std::shared_ptr<const snesim::SearchTree> loadOrCreateTree(const std::string& trainingImageName,
	...
	const snesim::SearchTree tree = snesim::buildTreeForLevel(trainingImage, summary, pathPositionArray, treeConfig);
...
int main(int argc, char const* argv[]) {
	...
	fprintf(reportFile,
		"[WSNESIM] tree creation workers=%u tasks=%lu\n",
		treeWorkerCount,
	...
	for (size_t levelPlanIndex = 0; levelPlanIndex < levelPlans.size(); ++levelPlanIndex) {
		...
		if (levelTreesIt == treesByLevel.end()
			...
			fprintf(reportFile, "[WSNESIM] incomplete tree set at level %u\n", levelPlan.level);
			if (options.closeReportFile) {
				fclose(reportFile);
			}
			return 0;
		...
		for (size_t tiIndex = 0; tiIndex < levelTreesIt->second.size(); ++tiIndex) {
			if (!levelTreesIt->second[tiIndex]) {
				fprintf(reportFile,
					"[WSNESIM] missing tree for TI '%s' at level %u\n",
