...
if __name__ == "__main__":
	...
	if os.path.exists('./kernelSet_Cat.npz') :
		data = numpy.load('kernelSet_Cat.npz')
	...
	def worker(queue, address):
		...
		while True:
			...
			# time.sleep(0.1)
			result=g2s('-sa',address,'-a','qs','-ti',source,'-di',numpy.nan*numpy.ones(shape=(250,250)),'-dt',numpy.ones(shape=(1,1)),'-ki',kernel,'-k',1.5,'-n',50,'-s',z,'-j',numberOfThreadProJob,'-silent','-noTO','-id',requestId);
