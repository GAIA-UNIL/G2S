...
def computeConnectivity(image,steps):
	...
	for i in range(len(steps)):
		...
		result[i]= (hist*(hist)).sum()/(image.size*(image.size-1))
	return result
...
if __name__ == "__main__":
	...
	if os.path.exists('./kernelSet_v4.npz') :
		data = numpy.load('kernelSet_v4.npz')
	...
	def saveData():
		print("save")
		numpy.savez('kernelSet_v4.npz', oldKernel=oldKernel, kernels=kernels, iteration=iteration, numberOfSimulation=numberOfSimulation, probPower=probPower, convergance=convergance, idValue=idValue)
	...
	def worker(queue, address):
		...
		while True:
			...
			# time.sleep(0.1)
			result=g2s('-sa',address,'-a','qs','-ti',source,'-di',numpy.nan*numpy.ones(shape=(200,200)),'-dt',numpy.zeros(shape=(1,1)),'-ki',kernel,'-k',1.5,'-n',50,'-s',z,'-j',numberOfThreadProJob,'-silent','-noTO','-id',requestId);
			qualityLocal[x,z]=mesureQualitry(varioRef, variogram(result[0]), connectivityRef, computeConnectivity(result[0],connectStep));
	...
	while maxIteration>iteration :
		...
		for _ in range(int(math.ceil(len(kernels)*muationfactor))):
			newKernels[perm[offset]]=mutateKernel(newKernels[perm[offset]],numpy.maximum(variability,1/(numpy.size(newKernels[perm[offset]]))*1/(1+iteration/10)))
			offset+=1
		# for _ in range(int(math.ceil(len(kernels)*muationfactor))):
