...
function loadQSannim(im){
  ...
  const sim_imageData = simcanvasCtx.getImageData(0, 0, simSize[0],simSize[1]);
	...
	function simIfNeeded(){
		...
		targetPsoition=Math.ceil(simLinearSize*targetSimProgression);
		if(targetPsoition==position || qsIsPause)
		{
			setTimeout(simIfNeeded,100);
			return
		}
		if(targetPsoition<position){
			position--;
			...
			sim_imageData.data[currentPosIndex*4+3]=20;
			...
			tierrorOverlay.animate({'opacity':1},600);
