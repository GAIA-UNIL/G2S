...
- **Direct Sampling (DS)** is a native CPU implementation of the Direct Sampling algorithm. It reuses the modern G2S simulation path and neighborhood infrastructure while using DS-style sequential TI scanning, threshold acceptance, best-so-far fallback, and optional local rotation/scale transforms on search-pattern offsets.
