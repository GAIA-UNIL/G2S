...
% QS call using G2S
[simulation,idx1,~]=g2s(param{:}, '-legacy_output');
...
id=0;
for i=1:5
    if(i>1)g2s('-kill',id, '-legacy_output');end
    id=g2s(param{:},'-submitOnly','-as',1,num2str(id), '-legacy_output');
    pause(2);
    
end

% Download data
[simulation2,idx2,~]=g2s('-waitAndDownload',id, '-legacy_output');  % '-kill' to interrupt a job
