...
jobid_1=g2s('-a','qs','-submitOnly','-ti',ti,'-di',nan(200,200),'-dt',[0],'-k',1.2,'-n',50,'-j',0.5, '-legacy_output');

% 2nd async call that waits for job 1 to finish using the "-after" flag
jobid_2=g2s('-a','qs','-after',jobid_1,'-submitOnly','-ti',ti,'-di',nan(200,200),'-dt',[0],'-k',1.2,'-n',50,'-j',0.5, '-legacy_output');
...
while status_2 < 95
    pause(2);
    status_1=g2s('-statusOnly',jobid_1, '-legacy_output');
    status_2=g2s('-statusOnly',jobid_2, '-legacy_output');
...
sim1=g2s('-waitAndDownload',jobid_1, '-legacy_output');
sim2=g2s('-waitAndDownload',jobid_2, '-legacy_output');
